...
RC forceFlushPool(BM_BufferPool *const bm) {
    ...
    for (int i = 0; i < bm->numPages; i++) {
        ...
        if (frame->fixCount == 0 && frame->dirtyFlag == true) {
            ...
            strcpy(pHandle->data, frame->frameData);
            openPageFile(bm->pageFile, &fHandle);
            writeBlock(pHandle->pageNum, &fHandle, pHandle->data);
...
RC forcePage (BM_BufferPool *const bm, BM_PageHandle *const page) {
    for (int i = 0; i < bm->numPages; i++) {
        ...
        if (frame->frameContents == page->pageNum) {
            SM_FileHandle fHandle;
            openPageFile(bm->pageFile, &fHandle);
            writeBlock(page->pageNum, &fHandle, frame->frameData);
            closePageFile(&fHandle);
            bm->mgmtData->numWriteIO++;
            return RC_OK;
        }
