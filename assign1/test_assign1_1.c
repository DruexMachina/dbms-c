...
testCreateOpenClose(void)
{
  ...
  ASSERT_TRUE((openPageFile(TESTPF, &fh, &md) != RC_OK), "opening non-existing file should return an error.");
...
testSinglePageContent(void)
{
  ...
  TEST_CHECK(readFirstBlock (&fh, ph, &md));
  ...
  for (i=0; i < PAGE_SIZE; i++)
    ph[i] = (i % 10) + '0';
  TEST_CHECK(writeBlock (0, &fh, ph, &md));
